...
def check_ollama():
    try:
        ollama.list()  # Pings the local Ollama server
    except Exception:
        print("ERROR: Ollama is not running.")
        print("Please install and start Ollama: https://ollama.com/download")
        print("Then run: ollama pull <your-model>")
        sys.exit(1)
        ...
        while True:
            ...
            # External commands
                ...
                if path:
                    ...
                    # Reconstruct the full sentence (e.g., "delete", "all", "logs") -> "delete all logs"
                    full_query = command + " " + " ".join(args)
                    print(f"🤖 AI Thinking: '{full_query}'...")
                    
                    generated_cmd = ai_engine.get_command_from_text(full_query)
                    
                    if "Error" in generated_cmd:
                        print(generated_cmd)
                    else:
                        print(f"⚡ Executing: {generated_cmd}")
                        # Auto-execute
                        try:
